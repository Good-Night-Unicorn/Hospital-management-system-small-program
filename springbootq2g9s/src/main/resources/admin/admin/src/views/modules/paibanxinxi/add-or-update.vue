<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="排班时间" prop="paibanshijian">
          <el-input v-model="ruleForm.paibanshijian" 
              placeholder="排班时间" clearable  :readonly="ro.paibanshijian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="排班时间" prop="paibanshijian">
              <el-input v-model="ruleForm.paibanshijian" 
                placeholder="排班时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="排班内容" prop="paibanneirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="排班内容"
                  v-model="ruleForm.paibanneirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.paibanneirong" label="排班内容" prop="paibanneirong">
                    <span>{{ruleForm.paibanneirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `paibanxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.paibanxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
