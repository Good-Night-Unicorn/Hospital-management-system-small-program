<template>
	<view class="content">
		<form class="app-update-pv">
			...
			<view :style='{"boxShadow":"","backgroundColor":"rgba(255, 255, 255, 0.56)","borderColor":"#ccc","margin":"","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
				<view :style='{"width":"160rpx","fontSize":"28rpx","color":"rgba(41, 40, 40, 1)","textAlign":"left"}' class="title">问诊时间</view>
				<input :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0.56)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(40, 38, 38, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' v-model="ruleForm.wenzhenshijian" placeholder="问诊时间" @tap="toggleTab('wenzhenshijian')"></input>
			</view>
			<view :style='{"boxShadow":"","backgroundColor":"rgba(255, 255, 255, 0.56)","borderColor":"#ccc","margin":"","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
			...
			<view :style='{"boxShadow":"","backgroundColor":"rgba(255, 255, 255, 0.56)","borderColor":"#ccc","margin":"","borderWidth":"0","borderStyle":"solid","height":"308rpx"}' class="cu-form-group">
				<view :style='{"width":"160rpx","fontSize":"28rpx","color":"rgba(28, 27, 27, 1)","textAlign":"left"}' class="title">问诊内容</view>
				<textarea :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 22, 22, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"280rpx"}' v-model="ruleForm.wenzhenneirong" placeholder="问诊内容"></textarea>
			</view>
		...
		<w-picker mode="dateTime" step="1" :current="false" :hasSecond="false" @confirm="wenzhenshijianConfirm" ref="wenzhenshijian" themeColor="#333333"></w-picker>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
        		this.ruleForm.wenzhenshijian = this.$utils.getCurDateTime();
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`zaixianwenzhen`, this.ruleForm.id);
		...
		methods: {
			...
			// 日长控件选择日期时间
			wenzhenshijianConfirm(val) {
				console.log(val)
				this.ruleForm.wenzhenshijian = val.result;
				this.$forceUpdate();
			},
			...
			async onSubmitTap() {
				...
				if(this.ruleForm.id){
					await this.$api.update(`zaixianwenzhen`, this.ruleForm);
				}else{
					await this.$api.add(`zaixianwenzhen`, this.ruleForm);
				}
